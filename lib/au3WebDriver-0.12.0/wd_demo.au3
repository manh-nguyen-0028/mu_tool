...
Func RunDemo($idDebugging, $idBrowsers, $idUpdate, $idHeadless, $idOutput)
	...
	ConsoleWrite("> Param: " & $idDebugging & "-" & $idBrowsers &"-" & $idUpdate &"-" & $idHeadless &"-" & $idOutput & @CRLF)
...
MsgBox($MB_ICONINFORMATION + $MB_TOPMOST, 'result #' & @ScriptLineNumber, $sElementSelector)
...
Func SetupChrome($bHeadless)
	_WD_Option('Driver', 'D:\Project\gtvn_auto_dv\chromedriver_win32_113\chromedriver.exe')
...
;~ 	Local $sCapabilities = '{"capabilities": {"alwaysMatch": {"goog:chromeOptions": {"w3c": true, "excludeSwitches": [ "enable-automation"]}}}}'
	...
	_WD_CapabilitiesAdd('args', 'user-data-dir', 'C:\Users\Administrator\AppData\Local\Google\Chrome\User Data\')
	_WD_CapabilitiesAdd('args', '--profile-directory', 'Profile 2')
